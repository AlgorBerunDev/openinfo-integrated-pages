...
<script setup>
  ...
  const selectedUrl = computed(() => {
    return `iuzse/trade-results/?page=${currentPage.value}&mkt_id=${mktId.value}${startDate?.value ? `&start_date=${moment(startDate?.value).format('YYYY-MM-DD')}` : ''}${endDate?.value ? `&end_date=${moment(endDate?.value).format('YYYY-MM-DD')}` : ''}${factSearchInput.value ? '&isu_cd=' + factSearchInput.value : ''}`
  })
