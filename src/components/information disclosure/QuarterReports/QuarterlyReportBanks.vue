...
<script setup>
  ...
  const getFacts = () => {
    ...
    axios
      .get(
        `${url}/reports/bank/quarter/?page=${currentPage.value}&report_type=2${dateValue.value.length ? `pub_date_lte=${dateValue.value[1].toISOString()}pub_date_gte=${dateValue.value[0].toISOString()}` : ''}&search=${factSearchInput.value}`,
        {
          method: 'get',
        },
      )
