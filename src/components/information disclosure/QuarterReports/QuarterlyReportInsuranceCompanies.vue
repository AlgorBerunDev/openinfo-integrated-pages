      ...
      <div v-for="item in data" class="facts-list">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <div class="d-flex gap-3 align-items-center">
            <a
              :href="`${url}/reports/export-excel/?report_type=${item?.type === 2 ? 'quarter' : 'annual'}&org_type=insurance&report_id=${item.id}&lang=${store.$state.currentLanguage}`"
              download
            >
              <img
                style="width: 26px"
            ...
            /></a>
            <a
              :href="`/reports/insurance_quarter/${item.id}`"
              class="button-more"
              color="#06AA17"
              target="_blank"
            >
...
<script setup>
  ...
  const getFacts = () => {
    ...
    axios
      .get(
        `${url}/reports/insurance/quarter/?page=${currentPage.value}&report_type=2${dateValue.value.length ? `pub_date_lte=${dateValue.value[1].toISOString()}pub_date_gte=${dateValue.value[0].toISOString()}` : ''}&search=${factSearchInput.value}`,
        {
          method: 'get',
        },
      )
