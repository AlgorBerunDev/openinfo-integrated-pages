...
<script setup>
  ...
  const getFacts = () => {
    ...
    axios
      .get(
        `${url}/reports/jsc/quarter/?page=${currentPage.value}${dateValue.value.length ? `&pub_date__lte=${dateValue.value?.[1]?.toISOString()}&pub_date__gte=${dateValue.value?.[0]?.toISOString()}` : ''}&search=${factSearchInput.value}`,
        {
          method: 'get',
        },
      )
