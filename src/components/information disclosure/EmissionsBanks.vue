  ...
  <el-container>
    <el-main v-loading="loading" class="fact-container">
      <div class="header-name title">
        <p>{{ $t('message.prospectuses-banks') }}</p>
      </div>
      ...
      <div v-for="item in data" class="facts-list">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <router-link
            class="button-more"
            color="#06AA17"
            :to="{ name: 'emission-banks-detail', params: { id: item.id } }"
        ...
        <div class="d-flex justify-content-between flex-wrap gap-3">
          <div class="d-flex gap-3 align-items-center justify-content-between">
            <p class="fact-date fw-bold text-primary">Дата раскрытия:</p>
          ...
          >
            <p class="fact-date text-primary fw-bold">
              Дата опубликования модератором:
            </p>
...
<script setup>
  ...
  const getFacts = () => {
    ...
    axios
      .get(
        `${url}/emissions/bank/?page=${currentPage.value}&report_type=2${dateValue.value.length ? `pub_date_lte=${dateValue.value[1].toISOString()}pub_date_gte=${dateValue.value[0].toISOString()}` : ''}&search=${factSearchInput.value}`,
        {
          method: 'get',
        },
      )
