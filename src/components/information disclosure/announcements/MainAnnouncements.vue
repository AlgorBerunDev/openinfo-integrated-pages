    ...
    <el-container>
      <el-main v-loading="loading" class="fact-container">
        <div class="search-bar">
          <el-select
            ref="mySelect"
            v-model="announcement_type"
            :placeholder="$t('message.all')"
            size="large"
            style="width: 450px"
            @change="selectAnnounceType"
          >
            <el-option
        ...
        <div v-for="fact in factLists" class="facts-list">
          <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="buttons d-flex gap-2 align-items-center">
              ...
              /></a>
              <a
                v-if="announcement_type !== 'charter'"
                target="_blank"
                class="button-more"
                color="#06AA17"
                :href="`/announce/${fact.id}/`"
              >
                {{ $t('message.more') }}
              </a>
            </div>
...
<script setup>
  ...
  // const currentReportOptions = computed(() => announceTypes[0].value)
    ...
    http
      .get(`/announcement/main/`, {
        params: {
          ...
          an_type: announcement_type.value,
        },
      })
      .then((response) => {
  ...
  const clearFields = () => {
    ...
    endDate.value = null
    announcement_type.value = announceTypes[0].value
    currentPage.value = 1
    fetchData()
  }
