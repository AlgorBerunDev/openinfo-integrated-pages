  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.licensing_decision_date') }}</th>
          <td>
            {{
        ...
        <tr>
          ...
          <td>
            {{
              new Date(data.date_licensing).toLocaleDateString('ru-RU', options)
            }}
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>{{ data.number_licensing }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.license_receipt_date') }}</th>
          <td>
            {{
        ...
        <tr>
          <th>{{ $t('message.license_validity_period') }}</th>
          <td>
            {{ data.validity_license ? data.validity_license : 'бессрочно' }}
          </td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact22/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
