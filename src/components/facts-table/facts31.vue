  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>
            {{ $t('message.the_issuers_body_that_made_the relevant_decision') }}
          </th>
          <td>{{ getReorganizationMethod(data.organ) }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.description_of_event') }}</th>
          <td>{{ data.description_issuers_obligations }}</td>
        </tr>
        ...
        <tr>
          <th>{{ $t('message.expiration_date_of_obligations') }}</th>
          <td>
            {{ new Date(data.date_end).toLocaleDateString('ru-RU', options) }}
          </td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact31/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
