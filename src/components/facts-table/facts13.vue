  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.the_body_that_imposed_the_arrest') }}</th>
          <td>{{ data.authority_imposing }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.date_of_seizure') }}</th>
          <td>
            {{
              new Date(data.date_seizure).toLocaleDateString('ru-RU', options)
            }}
          </td>
        </tr>
        <tr>
          <th>
            {{ $t('message.date_of_receipt_of_information_about_the_seizure') }}
          </th>
          <td>
            {{
              new Date(data.date_seizure_info).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
        ...
        <tr>
          <th>{{ $t('message.property_name') }}</th>
          <td>{{ getMeetingByType(data.name_of_property) }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.book_value_of_property_thousand_sum') }}</th>
          <td>{{ data.carrying_value }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.bank_account_details_with_arrest') }}</th>
          <td>{{ data.org_ticket_number }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.bank_name') }}</th>
          <td>{{ data.bank_name }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.account_number') }}</th>
          <td>{{ data.current_account }}</td>
        </tr>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact13/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
