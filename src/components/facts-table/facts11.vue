<template>
  ...
  <div v-loading="loading" class="loader">
    ...
    <h6 class="text-center text-dark">
      {{
        $t(
          'message.information_about_changes_in_list_of_affiliated_entities_DHO',
        )
      }}
    </h6>
    <el-table
      :data="data.factsinformationabout"
      border
    ...
    <el-table
      :data="data.factslistsubsiadiaries"
      border
      style="width: 100%; margin-bottom: 20px"
      :header-cell-style="{ color: '#fff', background: '#124483' }"
    >
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact11/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
