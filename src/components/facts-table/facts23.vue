  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.decision_maker') }}</th>
          <td>{{ getNameById(data.revocation_organ) }}</td>
        </tr>
        <tr>
          <th>
        ...
        <tr>
          <th>
            {{
              $t(
                'message.date_of_receipt_of_cancellation_or_termination_decision',
              )
            }}
          </th>
          <td>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact23/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
