  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>
            {{ $t('message.assets_at_end_of_previous_quarter_thousand_sum') }}
          </th>
          <td>
            {{
              data?.value_of_assets
                ?.toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ' ')
            }}
          </td>
    ...
    <el-table
      :data="data.factsinformationlist"
      border
    ...
    >
      <el-table-column
        prop="akt_name"
        :label="$t('message.name_of_the_asset')"
      />
      <el-table-column
        prop="pub_date"
        :label="$t('message.date_increase_in_assets')"
      >
      ...
      <el-table-column prop="sum" :label="$t('message.amount_thous_sums')">
        <template #default="scope">
          {{ scope.row.sum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ') }}
        </template>
      </el-table-column>
      <el-table-column prop="prosent" :label="$t('message.percent')" />
    </el-table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact17/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
