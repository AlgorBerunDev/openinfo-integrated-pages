  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.shareholder_full_name') }}</th>
          <td>{{ data.shareholder_fullname }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.shareholder_address') }}</th>
          <td>{{ data.shareholder_email }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.type_of_securities_entitling_redemption') }}</th>
          <td>{{ data.securities_type }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.nominal_value_per_share1') }}</th>
          <td>{{ data.securites_nominal_price }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.basis_for_redemption_right') }}</th>
          <td>{{ data.reasons_for_buy }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.decision_date_for_redemption_right') }}</th>
          <td>
            {{
              new Date(data.sollution_approval_date).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
        ...
        <tr>
          <th>{{ $t('message.redemption_price_per_security') }}</th>
          <td>{{ data.redemption_price }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.redemption_start_date') }}</th>
          <td>
            {{
              new Date(data.redemption_start_date).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
    ...
    <div class="social-links">
      <!--   <a :href="`https://openinfo.uz/ru/facts/to_pdf/${data.id}`">  <img src="/src/assets/images/icons/pdf.svg" alt="pdfIcon"/></a>-->
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact45/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
