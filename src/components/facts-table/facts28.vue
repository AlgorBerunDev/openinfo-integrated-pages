  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.reason_for_nonfulfillment') }}</th>
          <td>{{ data.suspension_reason }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.nonfulfillment_recognition_date') }}</th>
          <td>
            {{
              new Date(data.date_recognition_of_fail).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
        ...
        <tr>
          <th>
            {{ $t('message.issuer_receipt_date_of_invalidation_decision') }}
          </th>
          <td>
            {{
              new Date(data.date_solutions).toLocaleDateString('ru-RU', options)
            }}
          </td>
        </tr>

        <tr>
          <th>{{ $t('message.refund_procedure') }}</th>
          <td>{{ data.procedure_repayment }}</td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact28/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
