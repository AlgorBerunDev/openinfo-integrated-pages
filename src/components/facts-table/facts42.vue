<template>
  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      <thead>
        <tr>
          <th class="table-header text-white" colspan="12">
      ...
      <tbody>
        <tr>
          <th colspan="6">{{ $t('message.number_essential_fact') }}</th>
          <td colspan="6">{{ data.fact_number }}</td>
        </tr>

        <tr>
          <th colspan="6">{{ $t('message.name_essential_fact') }}</th>
          <td colspan="6">{{ data.fact_title }}</td>
        </tr>
        <tr>
          <th colspan="6">{{ $t('message.date_of_transaction') }}</th>
        ...
        <tr>
          <th colspan="6" rowspan="2">
            {{ $t('message.start_date_of_payment') }}
          </th>
          <td colspan="3" class="text-center">
            {{ $t('message.start_date') }}
          </td>
          <td colspan="3" class="text-center">{{ $t('message.end_date') }}</td>
        </tr>
        <tr>
          <td colspan="3" class="text-center">{{ data.payment_start_date }}</td>
          <td colspan="3" class="text-center">{{ data.payment_end_date }}</td>
        </tr>
        <tr>
          ...
          <td colspan="2" class="text-center">
            {{ $t('message.calculated') }}
          </td>
          <td colspan="2" class="text-center">{{ $t('message.paid') }}</td>
          <td colspan="2" class="text-center">
            {{ $t('message.outstanding') }}
          </td>
        </tr>
        <tr>
          <td class="text-center">{{ $t('.тыс.сум') }}</td>
          <td class="text-center">{{ $t('message.percent') }}</td>
          <td class="text-center">{{ $t('.тыс.сум') }}</td>
          <td class="text-center">{{ $t('message.percent') }}</td>
          <td class="text-center">{{ $t('.тыс.сум') }}</td>
          <td class="text-center">{{ $t('message.percent') }}</td>
        ...
        <tr>
          <td class="text-center">{{ data.overall_calculated_sum }}</td>
          <td class="text-center">{{ data.overall_calculated_percent }}</td>
          <td class="text-center">{{ data.overall_paid_sum }}</td>
          <td class="text-center">{{ data.overall_paid_percent }}</td>
          <td class="text-center">{{ data.overall_debt_sum }}</td>
          <td class="text-center">{{ data.overall_debt_percent }}</td>
        </tr>

        <tr>
          <th colspan="6">{{ $t('message.dividends_paid_amount') }}</th>
          <td class="text-center" colspan="6">
            {{ data.non_payment_explanation }}
          </td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact42/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
  ...
  const nthReorganizationMethod = (method) => {
    switch (method) {
      case 1:
        return 'наблюдение'
      case 2:
        return 'судебная санация'
      case 3:
        return 'мировое соглашение'
      case 4:
        return 'внешнее управление'
      default:
        return 'ликвидационное производство'
    }
  }
