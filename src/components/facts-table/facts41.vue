  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.meeting_of_the_Observation_Council') }}</th>
          <td>
            {{
              new Date(data.supervisory_board_protocol).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
        ...
        <tr>
          <th>{{ $t('message.Observation_Council_proposal') }}</th>
          <td>{{ data.supervisory_board_proposal }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.dividends_per_share') }}</th>
          <td>
            {{
              data.distrubution_offer_date
                ? new Date(data.distrubution_offer_date).toLocaleDateString(
                    'ru-RU',
                    options,
                  )
                : '-'
        ...
        <tr>
          <th colspan="2" style="text-align: center">
            {{ $t('Размер дивиденда  по простой акции ') }}
          </th>
        </tr>

        <tr>
          <th>{{ $t('message.In_sums_per_share') }}</th>
          <td>{{ data.distrution_share_sums }}</td>
        </tr>
        ...
        <tr>
          ...
          <td>{{ data.par_value_percent }}</td>
        </tr>

        <tr>
          <th colspan="2" style="text-align: center">
            {{ $t('message.Calculation_of_dividends_on_preferred_shares') }}
          </th>
        </tr>
        <tr>
          <th>{{ $t('message.In_sums_per_share') }}</th>
          <td>{{ data.dividend_share_sums }}</td>
        </tr>
        ...
        <tr>
          ...
          <td>{{ data.dividend_share_percent }}</td>
        </tr>
        <tr>
          <th colspan="2" style="text-align: center">
            {{ $t('message.calculation_of_income_on_other_securities') }}
          </th>
        </tr>
        <tr>
          <th>{{ $t('message.in_soums_per_security') }}</th>
          <td>{{ data.each_security_price }}</td>
        </tr>
        <tr>
          <th>
            {{
              $t('message.as_a_percentage_of_the_nominal_value_of_one_security')
            }}
          </th>
          <td>{{ data.each_security_nominal_percent }}</td>
        </tr>
      </tbody>
    </table>
    ...
    <table class="table table-hover table-bordered">
      <tbody>
        ...
        <!--    <tr>-->
        <!--      <th class="text-center" colspan="2"><a :href="data.fact_own_link">{{ $t("message.this_publication_is_available_on_the_Issuers_corporate_website") }}</a></th>-->
        <!--    </tr>-->
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact41/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
