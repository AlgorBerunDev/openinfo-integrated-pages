<template>
  ...
  <div v-loading="loading" class="loader">
    ...
    >
      <el-table-column
        prop="all_name"
        :label="$t('message.legal_entity_full_name')"
        align="center"
      />
      <el-table-column
        prop="position"
        :label="$t('message.location_postal_address_affiliate')"
        align="center"
      />
      <el-table-column
        prop="type_change"
        :label="$t('message.number_of_securities_shares')"
        align="center"
        width="200"
      />
      <el-table-column
        prop="view_paper"
        :label="$t('message.type_of_securities')"
        align="center"
        width="160"
      />
      <el-table-column
        prop="type_events"
        :label="$t('message.event_type')"
        align="center"
        width="120"
      >
        <template #default="scope">
          {{ scope.row.type_events === 0 ? 'исключен' : 'добавлен' }}
        </template>
      </el-table-column>
    </el-table>
      ...
      <tbody>
        <tr>
          <th class="text-center">
            {{
              $t('message.date_of_change_by_issuer_in_affiliated_persons_list')
            }}
          </th>
          <td class="text-center">
            {{ new Date(data.date_vnesn).toLocaleDateString('ru-RU', options) }}
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <el-table
      :data="data.factslistaffiliates"
      border
      style="width: 100%; margin-bottom: 10px"
      stripe
    >
      <el-table-column :label="$t('message.list_of_affilites')" align="center">
        <el-table-column
          prop="all_name"
          :label="$t('message.legal_entity_full_name')"
          align="center"
        ...
        <el-table-column
          prop="basis_name"
          :label="$t('message.grounds_on_which_they_are_recognized_affiliates')"
          align="center"
        />
        <el-table-column
          :label="$t('message.date_of_the_grounds')"
          align="center"
          width="120"
        >
          <template #default="scope">
            {{
              new Date(scope.row.date_ground).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact36/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
