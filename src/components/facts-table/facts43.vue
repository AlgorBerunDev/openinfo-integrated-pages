<template>
  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <tr>
          <th colspan="3">{{ $t('message.number_essential_fact') }}</th>
          <td colspan="3">{{ data.fact_number }}</td>
        </tr>

        <tr>
          <th colspan="3">{{ $t('message.name_essential_fact') }}</th>
          <td colspan="3">{{ data.fact_title }}</td>
        </tr>
        <tr>
          <th colspan="3">{{ $t('message.shareholder_full_name') }}</th>
          <td colspan="3">{{ data.shareholder_fullname }}</td>
        </tr>
        <tr>
          <th colspan="3">{{ $t('message.shareholder_address') }}</th>
          <td colspan="3">{{ data.shareholder_email }}</td>
        </tr>
        <tr>
          <th colspan="3">{{ $t('message.issuer_full_name') }}</th>
          <td colspan="3">{{ data.shareholder_full_title }}</td>
        </tr>
        <tr>
          <th colspan="3">{{ $t('message.date_of_transaction') }}</th>
        ...
        <tr>
          <th colspan="3" rowspan="2">
            {{ $t('message.number_of_shares_received') }}
          </th>
          <td class="text-center">
            {{ $t('message.number_of_securities_received') }}
          </td>
          <td class="text-center">{{ $t('message.percentages') }}</td>
          <td class="text-center">{{ $t('message.thousand_sum') }}</td>
        </tr>
        <tr>
          <td class="text-center">{{ data.securities_amount }}</td>
          <td class="text-center">{{ data.securities_percent }}</td>
          <td class="text-center">{{ data.securities_overall }}</td>
        </tr>

        <tr>
          <th colspan="3">{{ $t('message.notification_date') }}</th>
          <td class="text-center" colspan="3">
            {{
              new Date(data.notification_date).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact43/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
