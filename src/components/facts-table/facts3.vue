  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.reorganization_method') }}</th>
          <td colspan="2" style="color: #181818">
            {{ chooseReorganizationMethod(data.reorganization_method) }}
          </td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const chooseReorganizationMethod = (method) => {
    switch (method) {
      case 1:
        return 'слияние'
      case 2:
  ...
  const getFactsById = () => {
    loading.value = true
    axios
      .get(`${url}/disclosure/fact3/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
