<template>
  ...
  <div v-loading="loading" class="loader">
    ...
    <p class="text-dark mb-2 mt-4 text-center">
      Информация об измениях в списке юридических лиц, в которых эмитент
      обладает 10 и более процентами акций (долей, паев)
    </p>

    <el-table
      :data="data.factspercentofshares"
      border
    ...
    >
      ...
      >
        <template #default="scope">
          {{ getByType(scope.row.type_change) }}
        </template>
      </el-table-column>
    </el-table>

    <p class="text-dark mb-2 mt-4 text-center">
      Список юридических лиц, в которых эмитент обладает 10 и более процентами
      акций (долей, паев)
    </p>
    <el-table
      :data="data.factslistoflegal"
      border
      style="width: 100%; margin-bottom: 20px"
    >
      <el-table-column
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact30/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
