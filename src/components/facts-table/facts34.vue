  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.member_of_supervisory_board_name') }}</th>
        ...
        <tr>
          <td colspan="2">
            <el-table
              :data="data.factsshareinauthorized"
              border
              style="width: 100%"
            >
              <el-table-column
                :label="$t('message.share_in_authorized_capital_board_member')"
                align="center"
              >
                <el-table-column
                  :label="
                    $t('message.share_in_authorized_capital_before_change')
                  "
                  align="center"
                >
                  <el-table-column
                ...
                <el-table-column
                  :label="
                    $t('message.share_in_authorized_capital_after_change')
                  "
                  align="center"
                >
                  <el-table-column
                    prop="after_change_num"
                    label="кол-во"
                    align="center"
                  />
                  <el-table-column
        ...
        <tr>
          <td class="text-center">
            {{ $t('message.document_basis_of_changes') }}
          </td>
          <td class="text-center">{{ data.doc_emintet }}</td>
        </tr>

        <tr>
          <td colspan="2">
            <el-table
              :data="data.factslistofmembers"
              border
              style="width: 100%"
            >
              <el-table-column
                prop="all_name"
                :label="$t('message.member_of_supervisory_board_name')"
                align="center"
              />
              <el-table-column
                prop="share_ownership_quantity"
                :label="$t('message.share_ownership_quantity')"
                align="center"
              >
                <template #default="scope">
                  {{ scope.row.ownership_num }}
                </template>
              </el-table-column>
              <el-table-column
                prop="ownership_pro"
                :label="$t('message.share_ownership_percentage')"
                align="center"
              />
            </el-table>
          </td>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact34/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
