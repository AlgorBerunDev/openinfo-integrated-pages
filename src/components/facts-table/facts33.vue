  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.decision_date_bankruptcy') }}</th>
          <td>
            {{
        ...
        <tr>
          <th>{{ $t('message.invalidation_made_by') }}</th>
          <td>{{ getReorganizationMethod(data.organ) }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.bankruptcy_procedure') }}</th>
          <td>{{ nthReorganizationMethod(data.procedure) }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.decision_date') }}</th>
          <td>
            {{ new Date(data.date_begin).toLocaleDateString('ru-RU', options) }}
          </td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact33/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
