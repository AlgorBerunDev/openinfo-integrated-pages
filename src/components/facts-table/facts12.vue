  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>
            {{ $t('message.decision_making_body_creation_or_liquidation') }}
          </th>
          <td>{{ data.meeting_emintet }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.decision_date_creation_or_liquidation') }}</th>
          <td>
    ...
    <h6 class="text-center text-dark">
      {{
        $t(
          'message.information_about_changes_in_list_of_affiliated_entities_ZHO',
        )
      }}
    </h6>
    <el-table
      :data="data.factsinformationofabout"
      border
    ...
    >
      ...
      <el-table-column prop="type_change" :label="$t('message.Type_of_change')">
        <template #default="scope">
          {{ getMeetingByType(scope.row?.type_change) }}
        </template>
      </el-table-column>
    </el-table>

    <h6 class="text-center text-dark">
      {{ $t('message.list_of_dependent_business_entities_to_changes') }}
    </h6>

    <el-table
      :data="data.factslistsubsidiarie"
      border
      style="width: 100%; margin-bottom: 20px"
      :header-cell-style="{ color: '#fff', background: '#124483' }"
    >
      ...
      <el-table-column
        prop="type_change"
        :label="$t('message.ownership_percentage')"
        width="170"
        align="center"
      >
        <template #default="scope">
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact12/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
