  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.registration_date') }}</th>
          <td>
            {{
        ...
        <tr>
          <th>{{ $t('message.registered_foreign_stock_exchanges') }}</th>
          <td>{{ data.foreign_cap_markets }}</td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact38/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
