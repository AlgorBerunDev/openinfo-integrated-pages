  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.date_of_claim_submission_or_receipt') }}</th>
          <td>
            {{
              new Date(data.date_of_filing).toLocaleDateString('ru-RU', options)
            }}
          </td>
        </tr>

        <tr>
          <th>{{ $t('message.date_of_court_acceptance_to_proceedings') }}</th>
          <td>
            {{
              new Date(data.date_of_adoption).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
        ...
        <tr>
          <th>
            {{
              $t(
                'message.date_of_issuer_receipt_of_document_on_court_acceptance_and_appointment_of_hearing',
              )
            }}
          </th>
          <td>
            {{
              new Date(data.date_of_receipt).toLocaleDateString(
                'ru-RU',
                options,
              )
            }}
        ...
        <tr>
          <th>{{ $t('message.plaintiff') }}</th>
          <td>{{ data.person_filed }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.subject_of_claim') }}</th>
          <td>{{ data.subject_matter }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.claim_amount_sum') }}</th>
          <td>
            {{
              data?.claim_amount
                ?.toString()
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact14/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
