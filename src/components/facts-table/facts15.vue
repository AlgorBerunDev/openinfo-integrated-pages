  ...
  <div v-loading="loading" class="loader">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr>
          <th>{{ $t('message.issuer_decision_making_body') }}</th>
          <td>{{ data.body_of_issuer }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.loan_purpose') }}</th>
          <td>{{ data.purpose_loan }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.date_of_loan_receiving') }}</th>
          <td>
            {{
              new Date(data.date_receipt).toLocaleDateString('ru-RU', options)
            }}
          </td>
        </tr>

        <tr>
          <th>{{ $t('message.loan_amount_sum') }}</th>
          <td>
            {{
              data?.loan_amount
                ?.toString()
        ...
        <tr>
          <th>{{ $t('message.loan_size_from_authorized_capital_percent') }}</th>
          <td>{{ data.loan_capital }}</td>
        </tr>

        <tr>
          <th>{{ $t('message.credit_institution_name') }}</th>
          <td>{{ data?.name_of_bank }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.loan_repayment_term') }}</th>
          <td>{{ data?.maturity_loan }}</td>
        </tr>
        <tr>
          <th>{{ $t('message.collateral_or_guarantee_description') }}</th>
          <td>{{ data?.description_collateral }}</td>
        </tr>
      </tbody>
    </table>
...
<script setup>
  ...
  const getFactsById = () => {
    ...
    axios
      .get(`${url}/disclosure/fact15/${id.value}/`, {
        method: 'get',
      })
      .then((response) => {
