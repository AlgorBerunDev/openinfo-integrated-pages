<template>
  <div class="d-flex gap-3 mb-3 mt-3 align-items-center">
    ...
    <a
      ...
      target="_blank"
      :href="`${url}/announcement/announcement-submission-status/?${startDate ? `start_date=${moment(startDate).format(FactDateFormat)}` : ''}${endDate ? `&end_date=${moment(endDate).format(FactDateFormat)}` : ''}${searchInput ? `&search=${searchInput}` : ''}&submission_status=${statusOfOrganizations}&download=excel`"
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await http.get(
        `/announcement/announcement-submission-status/`,
        {
          params: {
