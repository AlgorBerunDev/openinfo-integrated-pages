<template>
  <div class="d-flex gap-3 mb-3 mt-3 align-items-center">
    ...
    <a
      ...
      target="_blank"
      :href="`${url}/disclosure/fact-submission-status/?${startDate ? `start_date=${moment(startDate).format(FactDateFormat)}` : ''}${endDate ? `&end_date=${moment(endDate).format(FactDateFormat)}` : ''}${selectValue ? `&fact_type=${selectValue}` : ''}${searchInput ? `&search=${searchInput}` : ''}&submission_status=${statusOfOrganizations}&download=excel`"
    ...
    >
      <el-table-column :label="t('message.trade.organization')" align="left">
        <template #default="scope">
          <a :href="scope.row.fact_url" style="color: #1256a0" target="_blank">
...
<script setup>
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await http.get(`/disclosure/fact-submission-status/`, {
        params: {
          ...
          end_date: endDate.value
            ...
            : '',
          fact_type: selectValue.value ? selectValue.value : '',
      ...
      console.error('Error fetching trade data:', error.response)
