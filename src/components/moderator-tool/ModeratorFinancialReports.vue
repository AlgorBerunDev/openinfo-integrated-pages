...
<script setup>
  ...
  const fetchData = async () => {
    ...
    let selectedUrl = ''
    if (report_type.value === 'quarterly') {
      selectedUrl = `/quarterly-submission-status/`
    } else if (report_type.value === 'annual') {
      selectedUrl = `/annual-submission-status/`
    }

    try {
      // Fetch data from the constructed URL
      const response = await http.get(`/reports${selectedUrl}`, {
        params: {
          year: selectedYear.value
            ? new Date(selectedYear?.value).getFullYear()
            : '',
          quarter:
            report_type.value === 'quarterly' ? selectedQuater.value : '',
        },
      })
      ...
      if (report_type.value === 'quarterly') {
        ...
        ConditionalRendering.value = false
      }

      statusOfOrganizations.value === 'submitted'
        ? (confirmedStatusOfOrganizations.value = true)
        : (confirmedStatusOfOrganizations.value = false)
