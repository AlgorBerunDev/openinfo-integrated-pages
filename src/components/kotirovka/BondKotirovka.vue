      ...
      >
        <template #default="scope">
          <p style="color: #1256a0">
            ...
            <!--            <el-tag style="margin-left: 4px">{{ scope.row.ticker }}</el-tag>-->
          </p>
        </template>
      </el-table-column>

      <el-table-column prop="ticker" align="center">
        <template #default="scope">
...
<script setup>
  ...
  const fetchTradeData = async (
    ...
    loading.value = true
    try {
      const response = await http.get(`iuzse/stock-screener/?mkt_id=BND`, {
        params: {
